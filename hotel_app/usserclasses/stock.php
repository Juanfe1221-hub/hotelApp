...
if ($action === 'list') {
    ...
    foreach ($productos as &$p) {
        ...
        if ($estadoCalc !== $p['estado']) {
            $upd = $pdo->prepare("UPDATE productos SET estado = :estado WHERE id = :id");
            $upd->execute([':estado' => $estadoCalc, ':id' => $p['id']]);
            $p['estado'] = $estadoCalc;
        }
    }
...
if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO productos (nombre, cantidad, unidad, precio, valor_venta, estado, inactivo, creado_por) 
                           VALUES (:nombre, :cantidad, :unidad, :precio, :valor_venta, :estado, :inactivo, 'admin')");
    $stmt->execute([
        ':nombre' => $_POST['nombre'],
        ':cantidad' => $_POST['cantidad'],
        ':unidad' => $_POST['unidad'],
        ':precio' => $_POST['precio'], // ahora es valor_compra
...
if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->execute([
        ...
        ':cantidad' => $_POST['cantidad'],
        ':unidad' => $_POST['unidad'],
        ':precio' => $_POST['precio'], // valor_compra
        ':valor_venta' => $_POST['valor_venta'],
        ':estado' => $estado,
        ':inactivo' => $inactivo
    ]);
...
<div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-4">
  <div class="d-flex align-items-center gap-3">
    <h3 class="m-0"><i class="fa-solid fa-boxes-stacked text-primary me-2"></i>Inventario</h3>
